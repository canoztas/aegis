...
class ModelExecutionEngine:
    ...
    def _build_runner(self, model: ModelRecord, role: Optional[ModelRole] = None):
        ...
        # Route to appropriate runner based on role
        if target_role == ModelRole.TRIAGE:
            return TriageRunner(provider, parser, config=model.settings)
        elif target_role == ModelRole.JUDGE:
            return JudgeRunner(provider, parser, config=model.settings)
        elif target_role == ModelRole.EXPLAIN:
            return ExplainRunner(provider, parser, config=model.settings)
        else:
            # Default to DeepScanRunner for DEEP_SCAN and CUSTOM roles
    ...
    async def run_models_concurrent(
        ...
        async def run_single_model(model: ModelRecord) -> tuple[str, List[Finding]]:
            ...
            try:
                runner = self._build_runner(model, role=model.roles[0] if model.roles else None)
                prompt = code
                ...
                async def execute():
                    return await runner.run(prompt, context)
    ...
    async def health_check_model(self, model: ModelRecord, timeout: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._with_timeout(runner.run(test_code, test_context), timeout=timeout)
