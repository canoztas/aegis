...
class PipelineRepository:
    ...
    def create(self, name: str, description: str, config: Dict[str, Any],
        ...
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO pipelines (name, description, config_json, is_default)
                VALUES (?, ?, ?, ?)
            """, (name, description, json.dumps(config), is_default))
            conn.commit()
            return cursor.lastrowid
    ...
    def get_by_name(self, name: str) -> Optional[Dict[str, Any]]:
        ...
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM pipelines WHERE name = ? AND enabled = 1", (name,))
            row = cursor.fetchone()
            if row:
                result = dict(row)
                if result.get('config_json'):
    ...
    def list_all(self) -> List[Dict[str, Any]]:
        """List all enabled pipelines."""
        db = get_db()
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM pipelines WHERE enabled = 1")
            results = []
            for row in cursor.fetchall():
                result = dict(row)
                if result.get('config_json'):
    ...
    def get_default(self) -> Optional[Dict[str, Any]]:
        ...
        with db.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM pipelines WHERE is_default = 1 AND enabled = 1 LIMIT 1")
            row = cursor.fetchone()
            if row:
                result = dict(row)
                if result.get('config_json'):
